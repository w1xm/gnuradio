...
def plot_2d(all_data, xaxis, yaxis='freqs', zaxis='data', normalized=False, cmap=TURBO_CMAP, savefolder=None):
...
def average_data(all_data, keys, velocity_correction=True):
    ...
    for col in all_data.columns.values():
        ...
        elif col.info.name == 'average_power':
            new_col = Column(
                name=col.info.name,
                data=(np.add.reduceat(
...
def plot(all_data, xaxes=None, yaxis=None, skip_1d=False, outlier_percentile=None, max_pointing_error=2*u.degree, recalculate_velocities=False, savefolder=None):
    ...
    zaxis = 'data'
    if yaxis not in ('freqs', 'vels'):
        zaxis = 'average_power'
    print("Z axis:", zaxis)

    normalized_data = None
    if not has_darksky and zaxis == 'data':
    ...
    for xaxis in xaxes:
        ...
        averaged_data = average_data(find_shift(raw_data, xaxis), average_keys)
        ...
        if has_darksky:
            averaged_data = average_data(calibrated_data, average_keys)
            plot_averaged(averaged_data, '_calibrated')
            plot_2d(averaged_data, xaxis, yaxis, zaxis=zaxis, normalized='calibrated', savefolder=savefolder)
        elif normalized_data:
            averaged_data = average_data(normalized_data, average_keys)
            plot_averaged(averaged_data, '_normalized')
            plot_2d(averaged_data, xaxis, yaxis, zaxis=zaxis, normalized='normalized', savefolder=savefolder)
