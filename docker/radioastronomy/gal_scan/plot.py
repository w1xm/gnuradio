...
AXIS_NAMES = {
    ...
    'skyoffset_longitude': 'Longitude Offset',
    'skyoffset_latitude': 'Latitude Offset',
...
def plot_observations(all_data, bad_data, savefolder=None):
    deg2rad = (2*np.pi)/360
    for (x, y) in (('longitude', 'latitude'), ('azimuth', 'elevation'), ('rci_azimuth', 'rci_elevation'), ('ra', 'dec'), ('skyoffset_longitude', 'skyoffset_latitude')):
        if x not in all_data.dtype.fields or y not in all_data.dtype.fields:
            continue
        plt.figure(figsize=(8,4.2))
        projection = "aitoff"
        if x == 'skyoffset_longitude':
            projection = None
        ...
        for data, marker, cmap in ((all_data, 'o', 'winter'), (bad_data, 'x', 'autumn')):
            ...
            if 'darksky' in data.dtype.fields:
                c = data['darksky']
            xaxis = Angle(data[x], unit=u.deg).wrap_at(180*u.deg)
            yaxis = Angle(data[y], unit=u.deg).wrap_at(90*u.deg)
            if projection:
                # Projections need radians
                xaxis, yaxis = xaxis.radian, yaxis.radian
...
def plot_2d(all_data, xaxis, yaxis='freqs', zaxis='data', cmap=TURBO_CMAP, savefolder=None):
    ...
    ylabel = "%s (%s)" % (AXIS_NAMES.get(yaxis,yaxis), all_data[yaxis].unit)
    xlabel = "%s (%s)" % (AXIS_NAMES.get(xaxis,xaxis), all_data[xaxis].unit)
    normalized = False
    if zaxis in ('calibrated_data', 'normalized_data'):
        normalized = zaxis[:-5]
    print('Plotting %s%s vs %s' % ((normalized+' ' if normalized else ''), xlabel, ylabel))
    if len(np.unique(all_data[xaxis])) == 1 or len(np.unique(all_data[yaxis])) == 1:
    ...
    def figure(name):
        ...
        yield fig
        if savefolder:
            plt.savefig(os.path.join(savefolder, '2d_'+xaxis+'_'+yaxis+'_'+name+suffix+'.png'), bbox_inches='tight')
            plt.close()
    ...
    contour_data = all_data[zaxis]
    # If zaxis is a scalar, we need to turn contour_data from 1D to 2D
    ...
    if ydata.shape != contour_data.shape:
        ydata = np.broadcast_to(ydata, reversed(contour_data.shape)).transpose()
    ...
    with figure('mesh'):
        ...
        mesh_ydata = center_mesh_coords(ydata.T).T
...
def oned2twod(xdata, ydata, zdata):
    xu, xi = np.unique(xdata, return_inverse=True)
    yu, yi = np.unique(ydata, return_inverse=True)
    zz = np.full((len(xu), len(yu)), np.nan) * zdata.unit
    for x, y, z in zip(xi, yi, zdata):
        zz[x, y] = z
    if np.isnan(zz).sum() < zz.size/2:
        # Seems like it's (mostly) a real grid; just return that grid.
        xdim = len(xu)
        ydim = len(yu)
        xu = np.broadcast_to(
            xu,
...
def center_mesh_coords(data):
    ...
    return np.append(extended, extended[:,-1:], axis=1)
...
def normalize_data(all_data, visualize=False):
    ...
    The returned normalized_data represents the estimated observed signal power.
    """
    ...
    all_data['normalized_data'] = all_data['data'] - correction_matrix
    return all_data
        ...
        if col.info.name in keep:
            ...
            if len(col.shape) > 1 and velocity_correction and 'vels' in all_data.columns:
                data = np.vstack(tuple(
                    average_point(group[col.info.name], group['vels'])
                    for group in groups
                ))
                new_col = Column(name=col.info.name, data=data)
            else:
                new_col = Column(
                    name=col.info.name,
                    data=(np.add.reduceat(
                        all_data[col.info.name],
                        i0s,
...
def apply_darksky(all_data):
    ...
    Returns:
        (observations with added calibrated_data column, darksky observations)
    """
    ...
    calibrated_obs = all_data[~all_data['darksky']]
    calibrated_obs['calibrated_data'] = calibrated_obs['data'] - darksky_obs['data']
    return calibrated_obs, darksky_obs
...
def plot(all_data, xaxes=None, yaxis=None, skip_1d=False, outlier_percentile=None, max_pointing_error=2*u.degree, min_elevation=None, recalculate_velocities=False, savefolder=None):
    ...
    if has_darksky:
        ...
        all_data, darksky_obs = apply_darksky(all_data)
    ...
    # Fill in default zaxis if unspecified
    old_plot_sets, plot_sets = plot_sets, []
    for p in old_plot_sets:
        if 'zaxis' in p:
            plot_sets.append(p)
        else:
    ...
    def plot_averaged(data, xaxis, yaxis, zaxis):
        suffix = '_' + zaxis
        if suffix.endswith('_data'):
            # Preserve old filenames
            suffix = suffix[:-5]
        for row in data:
            if yaxis == 'vels':
    ...
    for p in plot_sets:
        xaxis, yaxis, zaxis = p['xaxis'], p['yaxis'], p['zaxis']
        print("Plotting %s vs %s vs %s" % (xaxis, yaxis, zaxis))
        average_keys = [xaxis]
        if len(all_data[yaxis].shape) == 1:
            average_keys.append(yaxis)
        averaged_data = average_data(find_shift(all_data, xaxis), average_keys)
        if not p.get('skip_1d', skip_1d):
            print("Plotting 1D averaged %s" % (xaxis,))
            plot_averaged(averaged_data, xaxis, yaxis, zaxis)

        plot_2d(averaged_data, xaxis, yaxis, zaxis=zaxis, savefolder=savefolder)
